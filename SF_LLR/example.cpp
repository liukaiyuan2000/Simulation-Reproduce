...
// [[Rcpp::export]]
NumericMatrix llqr_cpp(NumericMatrix iC, NumericMatrix iCnew, NumericMatrix D, NumericVector Y, NumericVector h, int kernI, double tau) {
  int n = iC.nrow();
  int m = D.nrow();
  int J = iCnew.ncol();
...
// Local Linear Quantile Regression_cv_Single(only for one J)
// [[Rcpp::export]]
NumericMatrix llqr_cv_cpp(
  ...
  for (hi = 0; hi < nH; hi++) {
    ...
    for (i = 0; i < n; i++) {
      ...
      //std::cout << "Y(i)-Yt(0,0): " << rho_tau(temp, tau) << std::endl;
      CV(hi, k) += rho_tau(temp, tau);
    }
