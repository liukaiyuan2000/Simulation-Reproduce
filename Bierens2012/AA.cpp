...
arma::mat matreplace(arma::mat x) {
  return x.replace(arma::datum::nan, 1);
}

arma::mat replaceNAwithOne(arma::mat X) {
  arma::umat NA_indices = arma::find_nonfinite(X);
  X.elem(NA_indices).fill(1);
  return X;
}
//' @title A Rcpp version for Tnsc
//' @export
// [[Rcpp::export]]
double AA(arma::vec x, arma::vec y, arma::vec ytilde, double c) {
 int n = x.n_elem;
