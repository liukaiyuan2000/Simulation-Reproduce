...
arma::mat cor_rcpparma(arma::mat X) {
  int n = X.n_cols;
  arma::mat corrMatrix(n, n);
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      ...
      double meanX1 = arma::mean(x1);
      double meanX2 = arma::mean(x2);

      double sumXY = arma::dot(x1 - meanX1, x2 - meanX2);
      double sumX1Sq = arma::dot(x1 - meanX1, x1 - meanX1);
      double sumX2Sq = arma::dot(x2 - meanX2, x2 - meanX2);

      corrMatrix(i, j) = sumXY / sqrt(sumX1Sq * sumX2Sq);
    }
  }
