...
source("D:/Desktop/codes and literatures/陈美玲-加性异方差贝叶斯/Simulation_R/linear_funs.R")
...
}
{
  ...
  B.pre <-  scale(bs(x.pre, degree = 3, knots = mean(x.pre)), scale = F)
  y.pre <- B.pre %*% a2.hat
  plot(x.pre, 2*x.pre, type = 'l', ylim = range(c(2*x.pre, y.pre)))
  lines(x.pre, y.pre, col = "red", lwd = 2)
}
{
  ...
  B.pre <-  scale(bs(x.pre, degree = 3, knots = mean(x.pre)), scale = F)
  y.pre <- B.pre %*% a3.hat
  plot(x.pre, 3*x.pre, type = 'l', ylim = range(c(3*x.pre, y.pre)))
  lines(x.pre, y.pre, col = "red", lwd = 2)
}
