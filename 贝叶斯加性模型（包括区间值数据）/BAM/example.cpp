...
// [[Rcpp::export]]
double matdet(MatrixXd mat) {
  return mat.determinant();
}


MatrixXd eigenmattrans(NumericMatrix X) {
  
  Map<Eigen::MatrixXd> A(as<Map<Eigen::MatrixXd> >(X));
  
  return(A);
}

MatrixXd vec_to_rowmat(NumericVector vec) {
  Eigen::Map<Eigen::MatrixXd> mat(vec.begin(), 1, vec.size());
  
  return mat;
}

NumericMatrix rcppmattrans(MatrixXd X) {
...
MatrixXd matinv(NumericMatrix X) {
  MatrixXd mat = eigenmattrans(X);
  return mat.inverse();
}
...
NumericMatrix matmatmult(NumericMatrix A, NumericMatrix B) {
  MatrixXd mA = eigenmattrans(A);
  MatrixXd mB = eigenmattrans(B);
